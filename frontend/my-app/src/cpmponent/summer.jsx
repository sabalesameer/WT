...
const Summer = () => {
  ...
  // Fetch all cards and filter for the "summer" category
  useEffect(() => {
      ...
      fetch("https://mern-wt.onrender.com/api/cards/summer", {
        ...
        .then((data) => {
          if (Array.isArray(data) && data.length > 0) {
            const summerCards = data.filter((card) => card.category === "summer");
            if (summerCards.length > 0) {
              setCards(summerCards);
            } else {
              setFetchError("No summer cards available.");
            }
          } else {
  ...
  if (!cards || cards.length === 0) return <div>No summer cards available.</div>;

  return (
    <div className="absolute flex flex-wrap gap-20 top-20 justify-around p-10 left-0 z-[3] w-full">
      <div className="max-w-7xl mx-auto">
        {/* <h1 className="text-4xl font-bold text-center text-white mb-8">Summer Cards</h1> */}
