...
const Rain = () => {
  ...
  // Fetch all cards and filter for the "rain" category
  useEffect(() => {
      ...
      fetch("http://localhost:5000/api/cards/rain", {
        ...
        .then((data) => {
          if (Array.isArray(data) && data.length > 0) {
            const rainCards = data.filter((card) => card.category === "rain");
            if (rainCards.length > 0) {
              setCards(rainCards);
            } else {
              setFetchError("No rain cards available.");
            }
          } else {
  ...
  if (!cards || cards.length === 0) return <div>No rain cards available.</div>;

  return (
    <div className="absolute flex flex-wrap gap-20 top-20 justify-around p-10 left-0 z-[3] w-full">
      <div className="max-w-7xl mx-auto">
        {/* <h1 className="text-4xl font-bold text-center text-white mb-8">Rain Cards</h1> */}
